...
import 'package:om_mobile/feature/auth_login/model/login_response.dart';
import 'package:om_mobile/service/auth_manager.dart';
import 'package:om_mobile/core/models/master_user.dart';
...
class SessionController extends GetxController {
  ...
  Future<void> loadSessionData() async {
    ...
    // Debug: Log all available roles before filtering
    log("All available roles from API:");
    for (var rd in rolDepts) {
      log("  - roleId: ${rd.roleId}, roleDescr: '${rd.roleDescr}', deptId: ${rd.deptId}, deptName: '${rd.deptName}'");
    }
    ...
    // Debug: Log filtered roles
    log("Filtered allowed roles:");
    for (var rd in roleDeptMappings) {
      log("  - roleId: ${rd.roleId}, roleDescr: '${rd.roleDescr}', deptId: ${rd.deptId}, deptName: '${rd.deptName}'");
    }
    
    // Debug: Log all departments
    log("All departments:");
    for (var dept in depts) {
      log("  - deptId: ${dept.deptId}, deptName: '${dept.deptName}', deptCode: '${dept.deptCode}'");
    }
    
    // Debug: Log all roles from role master
    log("All roles from role master:");
    for (var role in roles) {
      log("  - roleId: ${role.roleId}, roleDescr: '${role.roleDescr}'");
    }
...
// 1. Resolve departments first
    ...
    debugPrint("data getting from $selectedDeptId---$selectedRoleId");
